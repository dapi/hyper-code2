...
export const RUN_ID = '2026-08-13-44225bf-runtime-authority-v6-1';
export const RAW = "R018-NONSECRET-e\u0301:'%/+/\u{10FFFF}";
...
export const SOURCE_PATHS = [...new Set([
    ...
    '.protocols/experiments/r018-sentinel-lib.ts',
])];
...
export function detections(value: unknown) {
    return detectSentinel(JSON.stringify(value), sentinelForms(RAW)).map((hit) => hit.formId);
}

export function detectorControls() {
    const sinks = ['requestHeader','authorityEnvelope','sqliteAuthorityRows','transcriptMessages','safeGateResult','brokerIPC','auditRecord','renderedEvents','transcriptRowsNotModelInput'];
    const positives = sentinelForms(RAW).flatMap((form) => form.values.flatMap((value) => sinks.map((sink) => ({ form: form.id, sink, detected: detectSentinel(value, [form]).length > 0 }))));
    return { positives, allPositive: positives.every((row) => row.detected), negative: detections({ digest: hash(RAW) }) };
}
...
function enqueueState(caseId: CaseId) {
    ...
    if (caseId.startsWith('use-') || caseId.startsWith('restart-') || caseId === 'failure-stale') {
        state.expiresAt = 200; state.revoked = false; state.replayed = false; state.generation = 2;
    }
    return state;
}

function transitionEnvelope(ctx: any, agentId: string, caseId: CaseId) {
    const updates: Record<string, [string, unknown[]]> = {
        'use-expired': ['UPDATE r032_v6_envelopes SET expires_at = 99 WHERE agent_id = ?', [agentId]],
        'use-revoked': ['UPDATE r032_v6_envelopes SET revoked = 1 WHERE agent_id = ?', [agentId]],
        'use-replay': ['UPDATE r032_v6_envelopes SET replayed = 1 WHERE agent_id = ?', [agentId]],
        'restart-stale': ['UPDATE r032_v6_envelopes SET generation = 1 WHERE agent_id = ?', [agentId]],
        'failure-stale': ['UPDATE r032_v6_envelopes SET generation = 1 WHERE agent_id = ?', [agentId]],
        'restart-recovery': ['UPDATE r032_v6_envelopes SET generation = 1 WHERE agent_id = ?', [agentId]],
    };
    const update = updates[caseId]; if (update) ctx.fns.db.exec(ctx, { sql: update[0], params: update[1] });
    if (caseId === 'restart-recovery') ctx.fns.db.exec(ctx, { sql: 'UPDATE r032_v6_envelopes SET generation = 2 WHERE agent_id = ?', params: [agentId] });
}

async function decide(candidate: Candidate, phase: 'enqueue' | 'use', state: ReturnType<typeof caseState>, brokerMode: 'memory' | 'child', ipc: unknown[], sandbox?: Sandbox): Promise<Decision & { rootCalls?: number }> {
    let brokerRootCalls = 0;
    ...
    if (candidate.broker) {
        ...
        ipc.push(request);
        const result = brokerMode === 'child' ? await childBroker(request, sandbox!) : decideBroker(request);
        ipc.push(result);
        if (!result.allow) return { ...result, principal: state.principal, scope: state.scope };
        brokerRootCalls = Number(result.rootCalls ?? 0);
    }
    return { allow: true, reason: controlled(candidate) ? `${phase}-authorized` : 'ambient-authority-gap', principal: state.principal, scope: state.scope, rootCalls: brokerRootCalls };
}

type Sandbox = { root: string; home: string; tmp: string };
async function childBroker(request: Parameters<typeof decideBroker>[0], sandbox: Sandbox) {
    const profile = `(version 1)(deny default)(allow process*)(allow file-read*)(allow file-write* (subpath "${sandbox.root}"))(deny network*)`;
    const child = Bun.spawn(['/usr/bin/sandbox-exec', '-p', profile, process.execPath, resolve(import.meta.dir, 'broker-child.ts')], {
        cwd: sandbox.root, env: { PATH: '/usr/bin:/bin', HOME: sandbox.home, TMPDIR: sandbox.tmp, R032_V6_CHILD: '1' }, stdin: 'pipe', stdout: 'pipe', stderr: 'pipe',
    });
...
async function runSuite(dbPath: string, brokerMode: 'memory' | 'child', sandbox?: Sandbox) {
    ...
    ctx.routes['/agent/:id'].POST = async (routeCtx: any, params: any, request: Request) => {
        ...
        const immediate = await decide(job.candidate, 'enqueue', state, brokerMode, ipc, sandbox);
    ...
    ctx.fns.agent.run = async (_runCtx: any, opts: any) => {
        ...
        if (job.caseId === 'mixed-principals-coalesced' && new Set(envelopes.map((row: any) => row.principal)).size > 1 && controlled(job.candidate)) {
            ...
            outcomes.get(opts.agent.id).root = { calls: 0, location: job.candidate.broker ? 'sandboxed-stdio-child' : 'parent-safe-root-spy' };
        ...
        const use = await decide(job.candidate, 'use', state, brokerMode, ipc, sandbox);
        const rootCalls = use.allow ? (job.candidate.broker ? Number(use.rootCalls ?? 0) : 1) : 0;
        outcomes.get(opts.agent.id).use = use;
        outcomes.get(opts.agent.id).root = { calls: rootCalls, location: job.candidate.broker ? 'sandboxed-stdio-child' : 'parent-safe-root-spy' };
        outcomes.get(opts.agent.id).worker = { claimRunState: claim.run_state, runStarted: claim.run_started_at != null, frontier, envelopeCount: envelopes.length };
    ...
    for (const candidate of CANDIDATES) {
        for (const caseId of CASES) {
            ...
            for (const principal of requests) {
                ...
                const headerSentinel = sentinelForms(RAW).find((form) => form.id === 'S3')!.values[0]!;
                const authorization = state.caller === 'missing' ? null : `Bearer ${headerSentinel}`;
            ...
            } else transitionEnvelope(ctx, agent.id, caseId);
            ...
            const sinks = {
                ...
                S7_render: events,
                S8_transcript_rows_not_model_input: messages.map((message: any) => ({ role: message.role, content: message.content })),
            };
...
export async function selfTest() {
    ...
    assert(result.rows.every((row) => row.sinks.S0_request.detections.length === 0 || row.sinks.S0_request.detections.includes('S3')));
    ...
    assert.equal(controls.allPositive, true);
    assert.equal(controls.negative.length, 0);
    ...
    assert.equal(row('CAN-01', 'immediate-missing').result.root.calls, 1);
    assert.equal(row('CAN-06', 'immediate-sufficient').result.root.location, 'sandboxed-stdio-child');
    assert.equal(row('CAN-06', 'immediate-sufficient').result.root.calls, 1);
...
async function collect(reviewPath: string) {
    ...
    const sandbox = { root: temp, home: resolve(temp, 'home'), tmp: resolve(temp, 'tmp') };
    await mkdir(sandbox.home); await mkdir(sandbox.tmp);
    const runDir = resolve(repo, '.protocols/experiments/runs/R-032', RUN_ID);
    try {
        const result = await runSuite(resolve(temp, 'carrier.sqlite'), 'child', sandbox);
        assert.equal(result.rows.length, 144);
        assert(CANDIDATES.every((candidate) => CASES.every((caseId) => result.rows.some((row) => row.candidate === candidate.id && row.caseId === caseId))));
        assert(result.rows.every((row) => Object.entries(row.sinks).filter(([name]) => name !== 'S0_request').every(([, sink]: any) => sink.detections.length === 0)));
        assert(result.rows.every((row) => !row.result?.use?.allow || row.result.root?.calls === 1));
        assert(result.rows.every((row) => row.result?.use?.allow || (row.result?.root?.calls ?? 0) === 0));
        const controls = detectorControls(); assert(controls.allPositive); assert.equal(controls.negative.length, 0);
        const artifacts: Record<string, unknown> = {
            'results.json': result.rows,
            'controls.json': controls,
            'containment.json': { sandbox, brokerReceipts: result.ipc.filter((row: any) => row?.containment), stopReceipts: result.stops, realNetwork: false, realListener: false },
            'manifest.json': { candidates: CANDIDATES, cases: CASES, committedEntries: 37, rows: 144, sinkStages: ['S0','S1','S2','S3','S4','S5','S6','S7','S8'], completeMatrix: false },
            'provenance.json': { runId: RUN_ID, head: FREEZE_HEAD, freezeSha256: pre.freezeSha256, review: pre.review, environment: pre.freeze.environment, stdout: 'captured per broker receipt', stderr: 'captured and required empty on success', stops: result.stops, sourceHashes: pre.freeze.sourceHashes, instrumentHashes: pre.freeze.instrumentHashes },
        };
        await mkdir(runDir, { recursive: false });
        for (const [name, value] of Object.entries(artifacts)) await writeFile(resolve(runDir, name), JSON.stringify(value, null, 2) + '\n');
        const rowHashes = result.rows.map((row, index) => ({ index, sha256: hash(JSON.stringify(row)) }));
        await writeFile(resolve(runDir, 'row-hashes.json'), JSON.stringify(rowHashes, null, 2) + '\n');
        const names = [...Object.keys(artifacts), 'row-hashes.json'].sort();
