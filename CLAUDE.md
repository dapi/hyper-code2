...
- Interactive `hcode` uses `src/cli/runTui.entry.ts` with imperative OpenTUI; redirected streams retain `runTerminal.entry.ts`. The TUI subscribes through agent-scoped `publishLive` / `subscribeLive` for non-durable deltas and always reconciles to SQLite events. Do not publish those deltas on the browser/global event bus.
