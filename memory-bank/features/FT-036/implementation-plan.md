...
| Evidence ID | Result | Carrier |
| --- | --- | --- |
| `EVID-06` | pass: loader receipts preserve exact non-enumerable function identity; namespace reload evaluates each winning logical function once; a whole-descriptor snapshot retries when source versions or precedence-preserving canonical scan membership changes and emits current membership without file-derived claims under continued churn; membership normalizes independent function ordering while retaining the exact ordered candidate sequence and multiplicity for each logical name; direct and concurrent replacement/deletion invalidates old provenance; final capability names and authority share one registry snapshot; cyclic registry namespaces are bounded; import cache collisions, ordinary rewrites and ABA rewrites fail closed | `src/loadFns.test.ts`, `src/repl/load.test.ts`, `src/self/describe.test.ts`; 36 targeted tests pass |
...
| `EVID-08` | pass: the listener remains on `0.0.0.0`; native IPv4 `127.0.0.0/8` is served, while the guard defensively recognizes dotted IPv4-mapped loopback and `::1`; malformed and non-loopback peers are rejected before descriptor evaluation | `src/http/$start.test.ts`, `src/self/$route_GET.test.ts` |
| `EVID-09` | pass locally with Bun 1.3.14: typecheck; 439 pass / 3 opt-in provider skips / 0 fail; Memory Bank lint and doctor errors 0; diff-check; two independent final re-reviews returned `CLEAN ACCEPT` with no critical or important findings | local command output; `.protocols/reviews/ft-036-implementation-review.md` |
